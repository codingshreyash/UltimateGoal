...
@Autonomous(name = "SRJA09", group = "")
@Disabled
public class SRJA09 extends OpMode {
...
//        angleReached = false;
    ...
    private boolean driveBackShoot(double distance, double drivePower) { //
        // set motors to run forward for 2500 encoder counts.
        double inchesToTicks = 40.584;
    ...
    private boolean startUpShooter(double distance, double shooterPower) {
        double inchesToTicks = 53.476;
        int ticks = (int) (distance * inchesToTicks);
        sMotor.setTargetPosition(ticks);
        sMotor.setMode(DcMotor.RunMode.RUN_TO_POSITION);
    ...
    private boolean shooterLoader(double distance, double shootingPower) {
        ...
        if (timer.milliseconds() >= 750 && servoMoving) {
    ...
    private boolean shooterLoader1(double distance, double shootingPower) {
        double inchesToTicks = 53.476;
        int ticks = (int) (distance * inchesToTicks);
        sMotor.setTargetPosition(ticks);
        sMotor.setMode(DcMotor.RunMode.RUN_TO_POSITION);
    ...
    private Boolean detectImage() {
        ...
        if (imageDetected.equals("Quad")) {
            ...
            autoSequence.add((Void) -> driveBackShoot(24, 60));
            autoSequence.add((Void) -> shooterLoader(50, 75)); //shoot1
            autoSequence.add((Void) -> shooterLoader(50, 67)); //shoot2
            autoSequence.add((Void) -> shooterLoader(50, 67)); //shoot3

            autoSequence.add((Void) -> driveDistanceBackwards(35, 60));
            autoSequence.add((Void) -> rotateLeft(63, 30));
